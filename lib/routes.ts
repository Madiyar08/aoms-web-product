 ...
 */
function computeEligibleAtms(schedule: { districts: string[] }): { eligible: ReturnType<typeof listAtms>; excludedByCategory: number; excludedNoCoords: number } {
  ...
  return { eligible, excludedByCategory, excludedNoCoords };
}
...
export async function buildRouteForSchedule(scheduleId: string): Promise<RouteEntry | { error: string }> {
  ...
  const { eligible: eligibleAtms, excludedByCategory, excludedNoCoords } = computeEligibleAtms(schedule);
  const excludedByStatus = 0; // всегда 0 — оставлено для обратной совместимости истории
