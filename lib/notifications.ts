...
export async function notifyCleaningReportToGroup(report: CleaningReport, atm: Atm | null): Promise<SendResult> {
  // Машина и экипаж — используем зафиксированный в отчёте machineId
  // напрямую (надёжный факт на момент отправки), а не поиск "у какой
  // машины сейчас числится этот сотрудник" — тот способ был той же
  // хрупкой связкой, что уже давала расхождения в отчётах, если
  // сотрудника позже пересаживали на другую машину. Старые отчёты без
  // machineId (до этого исправления) — запасной поиск по сотруднику.
  const machine = report.machineId
    ? listMachines().find((m) => m.id === report.machineId)
    : listMachines().find((m) => m.employee1Id === report.employeeId || m.employee2Id === report.employeeId);

  // Экипаж со своей отдельной Telegram-группой (например, сотрудники
  // Ташкентской области) — отчёт идёт туда, а не в общую группу.
  const chatId = machine?.telegramGroupChatId || process.env.TELEGRAM_GROUP_CHAT_ID || "";
  if (!chatId) return { ok: false, error: "Ни у экипажа, ни в общих настройках не задана группа для отчётов" };
