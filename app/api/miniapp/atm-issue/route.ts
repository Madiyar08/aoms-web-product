...
export async function POST(req: NextRequest) {
  ...
  const { initData, atmCode, address, latitude, longitude, reasons, otherText, comment, photos, correctDistrict, newAtmCode } =
    body;
  ...
  if (atm && latitude != null && longitude != null) {
    ...
    if (reasons.includes("ID банкомата изменился") && newAtmCode) {
      submitChangeRequest({
        atmId: atm.id,
        atmCode: atm.code,
        changeType: "id_change",
        newCode: String(newAtmCode).trim(),
        comment: String(comment || ""),
        photosJson: photoUrls.length > 0 ? JSON.stringify(photoUrls) : undefined,
        employeeId: employee.id,
        employeeName: employee.fullName,
