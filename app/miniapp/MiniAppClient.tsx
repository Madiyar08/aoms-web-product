...
export function MiniAppClient() {
  ...
  const [issuePrefill, setIssuePrefill] = useState<{ atmCode: string; address: string } | null>(null);
  ...
  if (showAudit) {
    return <AuditScreen initData={initData!} onBack={() => setShowAudit(false)} />;
  }
  ...
  if (showAtmIssue) {
    return (
      <AtmIssueScreen
        initData={initData!}
        prefillCode={issuePrefill?.atmCode}
        prefillAddress={issuePrefill?.address}
        onBack={() => {
          setShowAtmIssue(false);
  ...
  if (selectedAtm) {
    return (
      <ReportScreen
        ...
        onReportProblem={(atmCode, address) => {
          setIssuePrefill({ atmCode, address });
          setShowAtmIssue(true);
          setSelectedAtm(null);
        }}
      />
    );
  ...
  return (
    <div className="p-4 max-w-md mx-auto">
      ...
      {/* Экран выездной проверки — доступен только сотруднику с ролью
          "Проверяющий" (обычно сам руководитель или доверенное лицо).
          Поиск по ID БЕЗ ограничений маршрута/района (в отличие от
          обычного экрана сотрудника) — нужно найти любой банкомат
          системы для сверки на месте. */}
      {employeeRole === "Проверяющий" && (
        <button
          onClick={() => setShowAudit(true)}
          className="mt-3 w-full border-2 border-route bg-route-bg rounded-xl p-3 text-[13px] font-medium text-route"
        >
          🔎 Проверка банкомата
        </button>
      )}
...
function ReportScreen({
  ...
  onBack: () => void;
  onDone: () => void;
  onReportProblem: (atmCode: string, address: string) => void;
}) {
  ...
  const submittingRef = useRef(false); // синхронная защита от двойного клика — react-состояние обновляется не мгновенно
  ...
  async function submit() {
    // Синхронная защита от двойного нажатия — react-состояние
    // (submitting) обновляется не мгновенно, между кликом и
    // повторной отрисовкой кнопки есть доля секунды, за которую
    // быстрый повторный клик мог проскочить и создать дублирующий
    // отчёт. Ref читается и пишется сразу, без ожидания рендера.
  ...
  return (
    <div className="p-4 max-w-md mx-auto">
      ...
      {/* Раньше здесь была форма "Сообщить о проблеме", встроенная прямо в
          этот экран — убрали, чтобы не дублировать единую форму на
          главном экране. Но чтобы не заставлять сотрудника выходить и
          заново вводить тот же ID — эта кнопка открывает единую форму
          сразу с уже подставленным ID и адресом этого банкомата. */}
      {!isAdHoc && (
        <button
          onClick={() => onReportProblem(atm.code || enteredCode, atm.address)}
          className="mb-3 text-[12px] font-semibold text-amber-700 border border-amber-300 bg-amber-50 rounded-lg px-3 py-1.5"
        >
          ⚠️ Заметили проблему?
        </button>
      )}
...
function AtmIssueScreen({
  ...
  const [atmCode, setAtmCode] = useState(prefillCode || "");
  ...
  const [newAtmCode, setNewAtmCode] = useState("");
  ...
  return (
    <div className="p-4 max-w-md mx-auto">
      ...
      {reasons.includes("ID банкомата изменился") && (
        <div className="mb-3">
          <label className="block text-[12px] font-semibold text-gray-500 mb-1">
            Какой новый ID указан на банкомате сейчас?
          </label>
          <input
            value={newAtmCode}
            onChange={(e) => setNewAtmCode(e.target.value)}
            placeholder="Новый ID"
            className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm font-mono"
          />
          <p className="text-[11px] text-gray-400 mt-1">
            Старый ID уже указан выше — здесь введите новый, который сейчас на корпусе.
          </p>
        </div>
      )}
...
/**
 * Экран выездной проверки для роли "Проверяющий" — поиск ЛЮБОГО
 * банкомата по ID (без ограничений района/маршрута, в отличие от
 * обычного сотрудника), полная история очистки, заявки и статус
 * очереди изменений. Кнопка внизу отправляет сообщение лично
 * руководителю (TELEGRAM_MANAGER_CHAT_ID), не в общие группы.
 */
function AuditScreen({ initData, onBack }: { initData: string; onBack: () => void }) {
  const [mode, setMode] = useState<"id" | "date">("date");
  const [code, setCode] = useState("");
  ...
  async function search() {
    ...
    setSentResult(null);
    try {
      const res = await fetch("/api/miniapp/audit-lookup", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ initData, code: code.trim() }),
      });
      const data = await res.json();
      if (!data.ok) {
      ...
      setError("Не удалось выполнить поиск. Проверьте связь.");
    }
    setLoading(false);
  }
  ...
  async function sendToManager() {
    ...
    setSentResult(null);
    try {
      const res = await fetch("/api/miniapp/audit-report", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ initData, atmId: result.atm.id, message: message.trim() }),
      });
      const data = await res.json();
      setSentResult(data.ok ? "Отправлено ✓" : data.error || "Ошибка отправки");
      if (data.ok) setMessage("");
    } catch {
      setSentResult("Не удалось отправить. Проверьте связь.");
    }
    setSending(false);
  }
  ...
  return (
    <div className="p-4 max-w-md mx-auto">
      ...
      <h1 className="text-base font-semibold mb-3">🔎 Проверка</h1>

      <div className="flex gap-2 mb-4 bg-gray-100 rounded-lg p-1">
        <button
          onClick={() => setMode("date")}
          className={`flex-1 text-xs font-semibold rounded-md py-1.5 ${mode === "date" ? "bg-white shadow-sm" : "text-gray-500"}`}
        >
          По дате
        </button>
        <button
          onClick={() => setMode("id")}
          className={`flex-1 text-xs font-semibold rounded-md py-1.5 ${mode === "id" ? "bg-white shadow-sm" : "text-gray-500"}`}
        >
          По ID банкомата
        </button>
      </div>

      {mode === "date" && <AuditByDateFlow initData={initData} />}
      ...
      <div className="flex gap-2 mb-4">
        <input
          ...
          placeholder="Введите ID банкомата"
          className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm font-mono"
        />
        <button
          onClick={search}
          disabled={loading || !code.trim()}
          className="bg-ink text-white rounded-lg px-4 py-2 text-sm font-semibold disabled:opacity-40"
        >
          {loading ? "…" : "Найти"}
        </button>
      </div>
        ...
        <>
          <div className="border border-gray-200 rounded-lg p-3 mb-3">
            <div className="font-semibold text-sm mb-1">
              {result.atm.code} — {result.atm.name}
            </div>
            <div className="text-[12px] text-gray-500 mb-2">{result.atm.address}</div>
            <div className="grid grid-cols-2 gap-1 text-[12px]">
              <div>
                Район: <b>{result.atm.district}</b>
              </div>
              <div>
                Категория: <b>{result.atm.category}</b>
              </div>
              <div>
                Статус: <b>{result.atm.status}</b>
              </div>
              <div>
                Последняя очистка:{" "}
                <b>{result.atm.lastCleanedDate || "никогда"}</b>
              </div>
            </div>
            <div className="text-[11px] text-gray-400 mt-2">
              Координаты: {result.atm.latitude}, {result.atm.longitude}
            </div>
            {result.pendingChanges?.length > 0 && (
              <div className="mt-2 text-[11px] text-amber-600 bg-amber-50 rounded p-2">
                ⏳ {result.pendingChanges.length} заявок в очереди изменений ожидает подтверждения
              </div>
            )}
          </div>

          <div className="mb-3">
            <h2 className="text-[13px] font-semibold mb-1">История очистки (последние {result.history.length})</h2>
            {result.history.length === 0 ? (
              <p className="text-[12px] text-gray-400">Ни разу не очищался.</p>
            ) : (
              <div className="flex flex-col gap-1.5 max-h-52 overflow-y-auto">
                {result.history.map((h: any, i: number) => (
                  <div key={i} className="text-[11.5px] bg-gray-50 rounded p-2">
                    <div className="flex items-center justify-between">
                      <span className="font-medium">{h.employeeName}</span>
                      <span className="text-gray-400">{h.date}</span>
                    </div>
                    <div className="text-gray-500">
                      {h.inRoute ? "в маршруте" : "вне маршрута"}
                      {h.antifraudFlags?.length > 0 && (
                        <span className="text-red-500"> · {h.antifraudFlags.join("; ")}</span>
                      )}
                    </div>
                  </div>
                ))}
          ...
          {result.issues?.length > 0 && (
            <div className="mb-3">
              <h2 className="text-[13px] font-semibold mb-1">Заявки о проблемах</h2>
              <div className="flex flex-col gap-1.5">
                {result.issues.map((iss: any, i: number) => (
                  <div key={i} className="text-[11.5px] bg-gray-50 rounded p-2">
                    <div className="flex items-center justify-between">
                      <span>{iss.reasons.join(", ")}</span>
                      <span className="text-gray-400">{iss.date}</span>
                    </div>
                    <div className="text-gray-500">
                      {iss.employeeName} · {iss.status}
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="border-t border-gray-200 pt-3">
            <label className="block text-[12px] font-semibold text-gray-500 mb-1">
              Написать руководителю лично
            </label>
            <textarea
              value={message}
              onChange={(e) => setMessage(e.target.value)}
              placeholder="Что заметили на месте?"
              className="w-full border border-gray-300 rounded-lg px-3 py-2 text-sm mb-2"
              rows={3}
            />
            <button
              ...
              className="w-full bg-route text-white rounded-lg py-2.5 text-sm font-semibold disabled:opacity-40"
            >
              {sending ? "Отправка…" : "Отправить"}
            </button>
            {sentResult && <div className="mt-2 text-[12px] text-center">{sentResult}</div>}
          </div>
        </>
      )}
        </>
...
/**
 * Путь "Проверка по дате": дата → экипажи → районы (с охватом
 * задано/сделано/%) → список точек → детали конкретного отчёта (фото,
 * время, кто отправил). Реализует то, что описал руководитель — не
 * просто поиск одного банкомата, а обзор всей работы дня.
 */
function AuditByDateFlow({ initData }: { initData: string }) {
  ...
  async function loadCrews() {
    ...
    setPoints(null);
    try {
      const res = await fetch("/api/miniapp/audit-day", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ initData, date }),
      });
      const data = await res.json();
      setCrews(data.ok ? data.crews : []);
    } catch {
      setCrews([]);
    }
  ...
  async function loadPoints(crew: any, district: string) {
    setSelectedCrew(crew);
    setSelectedDistrict(district);
    setPoints(null);
    setLoadingPoints(true);
    try {
      const res = await fetch("/api/miniapp/audit-district-atms", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ initData, date, machineId: crew.machineId, district }),
      });
      const data = await res.json();
      setPoints(data.ok ? data.points : []);
    } catch {
      setPoints([]);
    }
  ...
  async function openReport(reportId: string) {
    ...
    try {
      const res = await fetch("/api/miniapp/audit-report-detail", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ initData, reportId }),
      });
      const data = await res.json();
      if (data.ok) setSelectedReport(data.report);
    } catch {
      /* тихо игнорируем — просто не откроется */
    }
    setLoadingReport(false);
  }

  // Экран деталей отчёта — поверх всего остального
  if (selectedReport || loadingReport) {
    return (
      <div>
        <button onClick={() => setSelectedReport(null)} className="text-sm text-gray-500 mb-3">
          ← Назад к списку
        </button>
        {loadingReport && <p className="text-sm text-gray-400">Загрузка…</p>}
        {selectedReport && (
          <div>
            <h2 className="text-sm font-semibold mb-1">
              {selectedReport.atmCode} {selectedReport.atmName && `— ${selectedReport.atmName}`}
            </h2>
            <div className="text-[12px] text-gray-500 mb-3">
              {selectedReport.employeeName} · {selectedReport.time}
              <br />
              {selectedReport.inRoute ? "в маршруте" : "вне маршрута"} · статус на месте:{" "}
              {selectedReport.reportedStatus || "не указан"}
            </div>
            {selectedReport.antifraudFlags?.length > 0 && (
              <div className="text-[11.5px] text-red-600 bg-red-50 rounded-lg p-2 mb-3">
                {selectedReport.antifraudFlags.join("; ")}
              </div>
            )}
            {selectedReport.photos?.length > 0 ? (
              <div className="grid grid-cols-2 gap-2">
                {selectedReport.photos.map((p: string, i: number) => (
                  <img key={i} src={p} alt="" className="w-full rounded-lg" />
                ))}
              </div>
            ) : (
              <p className="text-[12px] text-gray-400">Фото не приложено.</p>
            )}
          </div>
        )}
      </div>
  ...
  // Экран списка точек района
  if (selectedCrew && selectedDistrict) {
    return (
      <div>
        <button
          onClick={() => {
        ...
        <h2 className="text-sm font-semibold mb-3">
          {selectedCrew.machineNumber} — {selectedDistrict}
        </h2>
        {loadingPoints && <p className="text-sm text-gray-400">Загрузка…</p>}
        {points && points.length === 0 && <p className="text-sm text-gray-400">Точек не найдено.</p>}
        <div className="flex flex-col gap-1.5">
          {points?.map((p) => (
            <button
              key={p.atmId}
              onClick={() => p.reportId && openReport(p.reportId)}
              disabled={!p.reportId}
              className={`flex items-center justify-between text-left text-[12.5px] rounded-lg px-3 py-2.5 ${
                p.done ? "bg-green-50" : "bg-gray-50"
              }`}
            >
              <span>
                <span className={p.done ? "text-green-600" : "text-gray-400"}>{p.done ? "✓" : "○"}</span>{" "}
                <span className="font-mono">{p.code}</span> — {p.name}
              </span>
              {p.done && <span className="text-gray-400">→</span>}
            </button>
          ))}
        </div>
      </div>
  ...
  // Экран экипажей и районов на дату
  return (
    <div>
      <div className="flex gap-2 mb-4">
        <input
          ...
          className="flex-1 border border-gray-300 rounded-lg px-3 py-2 text-sm"
        />
        <button onClick={loadCrews} disabled={loadingCrews} className="bg-ink text-white rounded-lg px-4 py-2 text-sm font-semibold">
          {loadingCrews ? "…" : "Показать"}
        </button>
      </div>

      {crews && crews.length === 0 && (
        <p className="text-sm text-gray-400">На эту дату расписания нет.</p>
      )}

      <div className="flex flex-col gap-3">
        {crews?.map((crew) => (
          <div key={crew.scheduleId} className="border border-gray-200 rounded-lg p-3">
            <div className="text-sm font-semibold mb-0.5">Машина {crew.machineNumber}</div>
            {crew.crewNames && <div className="text-[11.5px] text-gray-500 mb-2">{crew.crewNames}</div>}
            {!crew.routeBuilt && (
              <div className="text-[11px] text-amber-600 mb-2">Маршрут на этот день ещё не построен.</div>
            )}
            <div className="flex flex-col gap-1.5">
              {crew.districts.map((d: any) => (
                <button
                  key={d.district}
                  onClick={() => loadPoints(crew, d.district)}
                  className="flex items-center justify-between bg-gray-50 rounded-lg px-3 py-2 text-left"
                >
                  <span className="text-[12.5px]">{d.district}</span>
                  <span className="text-[11px] text-gray-500">
                    {d.done}/{d.assigned} · {d.pct}%
                  </span>
                </button>
              ))}
            </div>
