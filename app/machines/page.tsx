...
import { createMachineAction, deleteMachineAction, updateMachineTelegramGroupAction } from "./actions";
      ...
      <div className="grid grid-cols-3 gap-3.5">
        {machines.map((m) => (
          <div key={m.id} className="bg-white border border-line rounded-[10px] p-4">
            ...
            <form action={updateMachineTelegramGroupAction.bind(null, m.id)} className="flex gap-1.5 mb-3">
              <input
                name="telegramGroupChatId"
                defaultValue={m.telegramGroupChatId || ""}
                placeholder="Своя Telegram-группа (необязательно)"
                className="input flex-1 !text-[11px] !py-1.5"
              />
              <button className="text-[11px] border border-line rounded-md px-2 bg-white">Сохранить</button>
            </form>
        ...
        <div className="bg-white border border-line rounded-[10px] p-4">
          ...
          <form action={createMachineAction} className="flex flex-col gap-2.5">
            ...
            <input name="telegramGroupChatId" placeholder="Своя Telegram-группа (необязательно)" className="input" />
